    ...
    internal class VieBoss
    {
        ...
        public static void Draw(SpriteBatch _spriteBatch)
        {
            if (!MapUI._showUI)
            {
                ...
                for (int y = 0; y < Jeu._listeBoss.Count; y++)
                {
                    ...
                    for (int i = 0; i < Jeu._listeBoss[y].Vie; i++)
                ...
                for (int y = 0; y < Jeu._listeBoss.Count; y++)
                {
                    for (int i = 0; i < VIE_MAX_FACILE_DIFFICILE - Jeu._listeBoss[y].Vie; i++)
                        _spriteBatch.Draw(_texturevieCoeurVide, new Vector2(pos.X + 10 + (float)(40 * Jeu._listeBoss[y].Vie) + 40 * i, pos.Y + 10 + (55 * y)), Color.White);
                }
            }
        }
    }
